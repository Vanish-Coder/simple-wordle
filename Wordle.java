...
{ 
	...
	public void drawPanel ( )
	{
		...
		StdDraw.picture(Constants.SCREEN_WIDTH / 2.0, Constants.SCREEN_HEIGHT - 20, "Logo.png");
	...
	public void respondToMouse ( )
	{
		if(readyForMouseInput && StdDraw.isMousePressed())
		{
			...
			{
				// Determine per-key hitbox. Make RESET (index 28) bigger so it's easier to click.
				int halfWidth = 22;
				int halfHeight = 29;
				if (i == 28) { // RESET key
					halfWidth = 200; //  wider
					halfHeight = 56; //  taller
				}
				if(StdDraw.mouseX() > Constants.KEYPLACEMENT[i][0] - halfWidth && 
					StdDraw.mouseX() < Constants.KEYPLACEMENT[i][0] + halfWidth && 
					StdDraw.mouseY() > Constants.KEYPLACEMENT[i][1] - halfHeight && 
					StdDraw.mouseY() < Constants.KEYPLACEMENT[i][1] + halfHeight)
				{
					if(i == 28)
					{
